...
const cityData = {
    Delhi: { coords: [28.61, 77.20], landmark: "India Gate" },
    Mumbai: { coords: [19.07, 72.87], landmark: "Gateway of India" },
    Paris: { coords: [48.85, 2.35], landmark: "Eiffel Tower" },
    London: { coords: [51.50, -0.12], landmark: "Big Ben clock tower" },
    Tokyo: { coords: [35.68, 139.69], landmark: "Shibuya Crossing" }
};
...
function calculate(city, aqi) {
    ...
    document.getElementById('f-sea').innerText = ["Mumbai", "London", "Tokyo"].includes(city) ? `+${(factor * mult * 0.4).toFixed(1)}m` : "STABLE";
    ...
    // AI IMAGE LOGIC (FIXED)
    const seed = Math.floor(Math.random() * 10000);
    const style = isHigh ? "dystopian%20apocalyptic%20heatwave%20smog%20flood" : "solarpunk%20futuristic%20green%20clean%20energy%20garden%20city";
    
    // Now using the optimized /p/ endpoint with a random seed to prevent caching
    document.getElementById('nowImg').src = `https://pollinations.ai/p/${city}%20${base.landmark}%20realistic%20photo%202024?width=600&height=400&nologo=true&seed=${seed}`;
    document.getElementById('futureImg').src = `https://pollinations.ai/p/${city}%20${year}%20${base.landmark}%20${style}?width=600&height=400&nologo=true&seed=${seed + 1}`;
